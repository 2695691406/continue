...
- [x] **新建** `core/llm/llms/AliyunCodingPlan.ts` — 主 Provider 类，继承 OpenAI
...
- [x] **修改** `core/llm/llms/index.ts` — 导入 AliyunCodingPlan 并添加到 `LLMClasses` 数组

### 3. OpenAI Adapters 层 (packages/openai-adapters/)

- [x] **修改** `packages/openai-adapters/src/index.ts` — 在 `constructLlmApi` 的 switch 语句中添加 `"aliyun-coding-plan"` case
  - 使用 `openAICompatible("https://coding.dashscope.aliyuncs.com/v1", config)` 即可

### 4. 自动检测与模板 (core/llm/autodetect.ts)

- [x] **修改** `core/llm/autodetect.ts` — 在 `PROVIDER_HANDLES_TEMPLATING` 数组中添加 `"aliyun-coding-plan"`
- [x] **修改** `core/llm/autodetect.ts` — 在 `PROVIDER_SUPPORTS_IMAGES` 数组中添加 `"aliyun-coding-plan"`（qwen3.5-plus 和 kimi-k2.5 支持图片理解）
- [x] **修改** `core/llm/autodetect.ts` — 在 `isProviderHandlesTemplatingOrNoTemplateTypeRequired` 函数中添加 qwen/minimax 模型名匹配
- [x] **修改** `core/llm/autodetect.ts` — 在 `PARALLEL_PROVIDERS` 数组中添加 `"aliyun-coding-plan"`

### 5. 模型信息 (packages/llm-info/)

- [x] **新建** `packages/llm-info/src/providers/aliyunCodingPlan.ts` — 定义模型信息列表
  - 包含 qwen3.5-plus, kimi-k2.5, glm-5, MiniMax-M2.5, qwen3-coder-next, qwen3-coder-plus 等
  - 标注 contextLength 和 maxCompletionTokens
- [x] **修改** `packages/llm-info/src/index.ts` — 导入并添加到 `allModelProviders`

### 6. GUI 配置面板 (gui/src/pages/AddNewModel/)

- [x] **修改** `gui/src/pages/AddNewModel/configs/models.ts` — 添加 Coding Plan 可用模型定义
- [x] **修改** `gui/src/pages/AddNewModel/configs/providers.ts` — 添加 `aliyun-coding-plan` provider 配置
...
- [x] 复用已有的 qwen.png 图标（Coding Plan 主推 Qwen 系列模型）

### 8. YAML 配置支持 (packages/config-yaml/)

- [x] 确认 provider 字符串 "aliyun-coding-plan" 被 schema 接受（使用 `z.string()` 无需修改）
